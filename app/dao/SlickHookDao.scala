...
case class DuplicateKeyException[X](key: X)
    extends Exception(s"A value already exists with key $key")
...
trait SlickHookDao[X, Y <: Hook[X], Z]
  ...
  protected def runKeyQuery(
      query: Query[Rep[String], String, Seq]
  ): Future[Seq[X]] = {
