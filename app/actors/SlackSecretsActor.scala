...
/** An actor which generates temporary secrets for Slack authorisation when
  * installing the app to a Slack workspace. Each secret is associated with a
  * Slack user ID, and the mapping is persisted to ensure that Slack
  * authorisation can recover in the case of a temporary outage.
  */
