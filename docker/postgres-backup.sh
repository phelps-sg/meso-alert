...
notification_msg="block-insights: postgres backup of $file_name to $S3_BUCKET FAILED"
...
if pg_dump --no-password -h $PG_HOST -p $PG_PORT -U $PG_USER --format=tar $PG_DATABASE | gzip -c >"$file_name"; then
  if aws s3 cp "$file_name" "$S3_BUCKET"; then
    notification_msg="$BACKUP_APP: postgres backup of $file_name to $S3_BUCKET SUCCESS"
  fi
fi
